...
def medir_latencia(host: str) -> float:
    """Mide la latencia ICMP hacia un host mediante el comando ping del SO."""
    ...
    try:
        ...
        if resultado.returncode == 0:
            # Regex compatible con Windows ("tiempo=Xms") y Linux/macOS ("time=X ms")
...
class VentanaConfiguracion(QDialog):
    """Diálogo para configurar la URL del backend y el token de autenticación del agente."""
    def __init__(self, parent_app):
        ...
        # QSettings persiste la configuración en el registro del SO (Windows) o archivo de preferencias (Linux/macOS)
    ...
    def guardar(self):
        ...
        # Inyectamos las credenciales al hilo de recolección sin necesidad de reiniciarlo
...
class VentanaMetricas(QDialog):
    """Ventana con el historial de las últimas 50 lecturas enviadas al backend."""
    def __init__(self):
        super().__init__()
...
class RecolectorThread(QThread):
    """Hilo secundario que recopila métricas de hardware cada 5 segundos y las envía al backend."""
    ...
    def __init__(self):
        ...
        # Caché para cálculos de tamaño de carpetas pesadas (recalculados cada 5 minutos)
        ...
        while self.is_running:
            ...
            if not self.is_paused:
                try:
                    ...
                    # Recalcular tamaños de carpetas del SO solo cada 5 minutos
                    # porque os.walk() es costoso en directorios grandes (ej. C:\Windows)
                    ahora_ts = time.time()
                    ...
                    for part in particiones:
                        if 'cdrom' in part.opts or part.fstype == '':
                            continue
                        # Excluir disqueteras antiguas (A:/B:) que aún aparecen en algunos equipos Windows
            ...
            # Espera fragmentada en intervalos de 1s para responder rápidamente a detener_por_completo()
            for _ in range(5):
...
class AgenteTrayApp:
    """Aplicación de bandeja del sistema que gestiona la interfaz del agente recolector."""
    def __init__(self):
