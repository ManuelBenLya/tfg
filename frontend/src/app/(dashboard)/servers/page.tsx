...
export default function ServersPage() {
  ...
  const handleDownloadPDF = async (servidorId: string, nombreServidor: string) => {
    try {
      const blob = await descargarReportePdf(servidorId);
  ...
  const handleEliminarServidor = async (servidorId: string, nombre: string) => {
    ...
    try {
      await eliminarServidor(servidorId);
      fetchServidores();
    } catch (error) {
  ...
  const handleRenombrarServidor = async (servidorId: string, nombreActual: string) => {
    ...
    try {
      await renombrarServidor(servidorId, nuevoNombre);
      fetchServidores();
    } catch (error) {
      console.error("Error al renombrar servidor:", error);
