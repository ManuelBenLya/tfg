...
import { asignarUsuariosServidor } from '@/services/servidorService';
import { getEmpleados } from '@/services/authService';
...
export default function ModalAsignarUsuarios({ servidorId, nombreServidor, usuariosConAcceso, onClose }: ModalAsignarUsuariosProps) {
  ...
  useEffect(() => {
    const fetchDatos = async () => {
      try {
        ...
        // Excluimos admins de la lista porque ya tienen acceso global implícito a todos los servidores
  ...
  const handleGuardar = async () => {
    setSaving(true);
    try {
      await asignarUsuariosServidor(servidorId, seleccionados);
      onClose();
    } catch (error) {
      console.error("Error al guardar asignación de usuarios", error);
    } finally {
      setSaving(false);
    }
  };
