...
def generar_conclusion_simbolica(cpu_avg, ram_avg, disco_avg):
    ...
    if ram_avg > 90:
        problemas.append("La memoria RAM está casi al límite de su capacidad, existe riesgo de uso excesivo de paginación (Swap).")
...
def crear_reporte_pdf(servidor, metricas_resumen, usuario_email):
    ...
    pdf.cell(0, 6, f"Uso medio de RAM: {ram}% (Umbral: {servidor.umbral_ram}%)", ln=True)
    ...
    diagnostico = generar_conclusion_simbolica(cpu, ram, disco)
