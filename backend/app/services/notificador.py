...
def enviar_notificaciones_alerta(db, servidor: Servidor, mensaje: str):
    """
    Distribuye notificaciones de alerta por todos los canales configurados
    (Discord, Slack, Email) a los usuarios con permisos sobre el servidor.
    Destinatarios: administradores de la empresa + técnicos asignados al servidor.
    """
    ...
    # Dict comprehension por ID para deduplicar usuarios que sean admin y estén asignados
    ...
    for user in usuarios_a_notificar:
        ...
        if user.recibir_alertas_email is not False:
            ...
            # Cascada de configuración SMTP: empresa > global (.env) > por defecto
            ...
            if not smtp_user or not smtp_password or not smtp_host:
                ...
                try:
                    ...
                    # Brevo expone una API HTTP alternativa a SMTP, útil cuando el hosting
                    # bloquea los puertos SMTP salientes (ej. Render Free Tier)
                    if "brevo" in smtp_host.lower():
                        ...
                        # Puerto 465 usa SSL directo; 587 usa STARTTLS (negociación sobre texto plano)
                        if smtp_port == 465:
